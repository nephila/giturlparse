...
class GitLabPlatform(BasePlatform):
    PATTERNS = {
        "https": r"https://(?P<domain>.+)(?P<port>:[0-9]+)?/(?P<owner>.+)/" r"(?P<repo>.+?)(?:\.git)?$",
        "ssh": r"(ssh://)?git@(?P<domain>.+):(?P<port>[0-9]+/)?(?P<owner>.+)/" r"(?P<repo>.+?)(?:\.git)?$",
        "git": r"git://(?P<domain>.+):(?P<port>[0-9]+)?/(?P<owner>.+)/" r"(?P<repo>.+?)(?:\.git)?$",
    }
    FORMATS = {
